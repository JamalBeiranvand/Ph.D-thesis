...
%  clc
%  warning off
function Performance_vs_iterations=Mainfunction_UpdateF_covergence(HT,NRF,P,Max_iteration)
...
%Initialize F and V for all n
   ...
   F=rand(size(F))+1j*rand(size(F));
   Vt=rand(size(Vt))+1j*rand(size(Vt));
%% Iterative Algorithm 
...
%        toc
       ...
       % disp(R/N) ;
        Performance_vs_iterations(index)=R/N;
        % Non-decreasing
        if R < sum_rate
            R = sum_rate;
            Performance_vs_iterations(index:Max_iteration)=R/N;
