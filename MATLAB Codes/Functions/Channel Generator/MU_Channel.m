function [H,At,Ar,alpha]=MU_Channel(Tx,K,Ncl,varargin)
...
MuH  = zeros(Nr,Nt,K);       % Channel matrix for realization repeat will be=  (Nr*Nt*realization)
 ...
 %%   Path by Uniform distribution  
    if  Nray ~=1       
        for k = 1:K
            ...
            Alpha=gamma*diag(alpha(:,k));
            MuH(:,:,k)=Ar(:,:,k)*Alpha*At(:,:,k)';
        end
    end
 %% 
    if  Nray ==1       
        for k = 1:K
            ...
            Ar(:,:,k)=sqrt(1/Nr)*exp(1i* pi*(RXy.*sin(Phi_r).* sin(Theta_r)+ RXz.*cos(Theta_r) ));
            alpha(:,k)=sqrt(1/2)*(randn(L,1) + 1j*randn(L,1));
%             alpha(:,k)=(randn(L,1) + 1j*randn(L,1));
            Alpha=gamma*diag(alpha(:,k));
            MuH(:,:,k)=Ar(:,:,k)*Alpha*At(:,:,k)';
        end
    end
