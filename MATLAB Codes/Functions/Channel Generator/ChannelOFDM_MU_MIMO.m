function [H,At,Ar,alpha]=ChannelOFDM_MU_MIMO(Tx,Rx,Ncl,K,F,varargin)
...
%     addParameter(p,'Distribution',DefaulDistribution,@(x) any(validatestring(x,ExpectedDistribution)));
    parse(p,Tx,Rx,Ncl,K,F,varargin{:});
...
H  = zeros(Nr,Nt,K,F);       % Channel matrix for realization repeat will be=  (Nr*Nt*realization)
