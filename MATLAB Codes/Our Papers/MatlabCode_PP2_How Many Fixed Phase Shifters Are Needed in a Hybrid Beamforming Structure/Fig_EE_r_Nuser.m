...
% [Setb5,Sb5]=GenSetModified(NPS(5));
for r=1:length(KM)
    disp(r)
    K=KM(r);
    Snr=SNR;
    E1=SNR+(200+K*300+Nt*K*NPS(1)*5+NPS(1)*K*5)*10e-3;
    E2=SNR+(200+K*300+Nt*K*NPS(2)*5+NPS(2)*K*5)*10e-3;
    E3=SNR+(200+K*300+Nt*K*NPS(3)*5+NPS(3)*K*5)*10e-3;
    E4=SNR+(200+K*300+Nt*K*NPS(4)*5+NPS(4)*K*5)*10e-3;
...
Y_points=[ .45*(diff(ax.YLim)) .57*(diff(ax.YLim))]+ax.YLim(1);
str1=['$N_t=$ ',num2str(Nt)];
