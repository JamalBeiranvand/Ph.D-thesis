...
K_vector=10:2:20;                           % number of users
...
Nsk=1;
Pc=SNR; 
 for r=1:length(K_vector)
    disp(r)
    K=K_vector(r);
    NrfTx =K;                    % Number of RF chains at BS
...
X_points=[16 19];
Y_points=[ .79*(diff(ax.YLim)) .96*(diff(ax.YLim))];
str1=['Tx Array: ',num2str(Tx(1)),'$\times$',num2str(Tx(2))];
%str2=['K: ',num2str(K)];
str3='$N_{\mathrm{RF}}=K$ ';
str4=['$N=64$ '];
Not1.String={str1,str3,str4};
Not1.FontSize = 10;
