...
Nrf =K:5:3*K;                     % Number of RF chains at BS
  ...
  % H=sqrt(1/2)*(randn(1,Nt,K,F)+1j*randn(1,Nt,K,F));
    for r=1:length(Nrf)
       ...
       % R_ZF(r,reali)=SumRate_MUMIMO_Hybrid(H,WBB,WRF,eye(Nt),FZF,eye(Ns));
        ...
        [FRF_S,FBB_S]=Sohrabi_OFDM_MISO(H,NrfTx,Pc);
        R_Sohrabi(r,reali)=SumRate_MUMIMO_Hybrid(H,WBB,WRF,FRF_S,FBB_S,eye(Ns));
        
%          %S1=0;S2=0;S3=0;
%         for i=1:size(V_chen,3)
%             for j=1:size(V_chen,4)
%                 S1(i,j)=sum(abs(FRF_DPS*FBB_DPS(:,1,i,j)).^2);
%                 S2(i,j)=sum(abs(F_chen*V_chen(:,1,i,j)).^2);
