...
NrfTx =K+1;                     % Number of RF chains at BS
...
for reali=1:realization
   ...
   %  H=sqrt(1/2)*(randn(1,Nt,K,F)+1j*randn(1,Nt,K,F));
    for r=1:length(SNR_dB)
        Pc=SNR(r); %power per each Subcarrier
        WRF=repmat(eye(Nr),1,1,K);
        WBB=repmat(eye(Nr),1,1,K,F);
        %FZF=WMMSE_MISO_OFDM(H,Snr);

        % ZF
        %FZF=ZF_MU_MC_MISO(H,Pc);
       % R_ZF(r,reali)=SumRate_MUMIMO_Hybrid(H,WBB,WRF,eye(Nt),FZF,eye(Ns));
        
           %DPS
        [WBB_DPS,WRF_DPS,FRF_DPS,FBB_DPS]=DPS_OFDM_MU_MIMO(H,NrfTx,1,Ns,Pc/((K*Nsk)));
        R_DPS(r,reali)=SumRate_MUMIMO_Hybrid(H,WBB_DPS,WRF_DPS,FRF_DPS,FBB_DPS,eye(Ns));
%            
        [F_chen,V_chen,OptPerf]=Chen_Hybrid(H,NrfTx,Pc);
        R_chen(r,reali)=SumRate_MUMIMO_Hybrid(H,WBB,WRF,F_chen,V_chen,eye(Ns));
%         
        [Fupdated_chen,V_chen_Fupdated,OptPerf]=Mainfunction_UpdateF(H,NrfTx,Pc);
        R_chen_updateF(r,reali)=SumRate_MUMIMO_Hybrid(H,WBB,WRF,Fupdated_chen,V_chen_Fupdated,eye(Ns));

        R_MMSE(r,reali)=OptPerf;

       [FRF_S,FBB_S]=Sohrabi_OFDM_MISO(H,2*NrfTx,Pc);
       R_Sohrabi(r,reali)=SumRate_MUMIMO_Hybrid(H,WBB,WRF,FRF_S,FBB_S,eye(Ns));
        
         %S1=0;S2=0;S3=0;
%         for i=1:size(V_chen_Fupdated,3)
%             for j=1:size(V_chen_Fupdated,4)
%                 S1(i,j)=sum(abs(F_chen*V_chen(:,1,i,j)).^2);
%                 S2(i,j)=sum(abs(Fupdated_chen*V_chen_Fupdated(:,1,i,j)).^2);
%                % S3(i,j)=sum(abs(FRF_S*FBB_S(:,1,i,j)).^2);
%             end
%         end

    end
end
Data1= sum((R_MMSE),2)/reali
Data2= sum(R_chen_updateF,2)/reali
Data3= sum(R_chen,2)/realization;
...
X_points=[-9.5 -4];
Y_points=[ .42*(diff(ax.YLim)) .65*(diff(ax.YLim))];
