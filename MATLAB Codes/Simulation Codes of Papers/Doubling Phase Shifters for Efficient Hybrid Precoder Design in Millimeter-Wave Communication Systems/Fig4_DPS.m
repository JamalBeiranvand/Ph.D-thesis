...
for r=1:length(SNR_dB)
    ...
    for reali=1:realization
       ...
       [WBD,FBD]=BD(H,Ns,Snr);
       [WBB_DPS,WRF_DPS,FRF_DPS,FBB_DPS_BD,FBB_DPS]=DPS_OFDM_MU_MIMO(H,NrfTx,NrfRx,Ns,Snr);
       %FBB_DPS_BD= BD_Hybrid(H,WBB_DPS,WRF_DPS,FRF_DPS,FBB_DPS,Snr);
%        
       [WBB,WRF,FRF,FBB]=DPS_Enabled_SPS_OFDM_MU_MIMO(FBD,WBD,NrfTx,NrfRx,Ns,Snr);
       FBB_BD=BD_Hybrid(H,WBB,WRF,FRF,Snr);
        ...
        R_DPS_BD(r,reali)=SumRate_MUMIMO_Hybrid(H,WBB_DPS,WRF_DPS,FRF_DPS,FBB_DPS_BD,eye(Ns));
        R_DPS_SPS(r,reali)=SumRate_MUMIMO_Hybrid(H,WBB,WRF,FRF,FBB_BD,eye(Ns));
       
    end
end
Data1= sum((R_BD),2)/realization;
Data2= sum(R_DPS_BD,2)/realization;
Data3= sum(R_DPS_SPS,2)/realization;
 Data4= sum(R_DPS,2)/realization;
...
Legend1='Fully Digital BD Precoder';
Legend2='DPS';
Legend3='DPS-enabled SPS';
 Legend4='DPS w/o BD';
...
ax.XLim = [min(Xaxis) max(Xaxis)];
  ax.YLim = [40 120];
ax.XTick = min(Xaxis):2: max(Xaxis);
% ax.YTick = 0:5:50;
% ax.XTickLabel=Xaxis;
