...
NRFTx = 9:14;                     % Number of RF chains at BS
...
R_MMSE=zeros(length(NRFTx),realization);
R_ZF=zeros(length(NRFTx),realization);
R_MRC=zeros(length(NRFTx),realization);
Novelmmse=zeros(length(NRFTx),realization);
Novelzf=zeros(length(NRFTx),realization);
Novelmrc=zeros(length(NRFTx),realization);
R_Shr=zeros(length(NRFTx),realization);
R=zeros(K,1);
%%
...
for reali=1:realization
       ...
       [WBD,FBD]=BD(H,Ns,SNR);
    for r=1:length(NRFTx)
       NrfTx=NRFTx(r);             
       [WBB,WRF,FRF,FBB]=DPS_OFDM_MU_MIMO(FBD,WBD,NrfTx,NrfRx,Ns,SNR);
       FBB_BD= BD_Hybrid(H,WBB,WRF,FRF,FBB,SNR);     
       R_BD(r,reali)=SumRate_MUMIMO_Hybrid(H,WBD,repmat(eye(Nr),1,1,K),eye(Nt),FBD,eye(Ns));
       R_DPS(r,reali)=SumRate_MUMIMO_Hybrid(H,WBB,WRF,FRF,FBB,eye(Ns));
       R_DPS_BD(r,reali)=SumRate_MUMIMO_Hybrid(H,WBB,WRF,FRF,FBB_BD,eye(Ns));
    end
end
...
% ax.YTick = 0:5:50;
ax.XTickLabel=Xaxis;
ax.XLabel.String='Number of RF chains at Tx';
