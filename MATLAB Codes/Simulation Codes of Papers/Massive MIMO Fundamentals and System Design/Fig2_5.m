...
SNR_dB = -10:4:30;             % Signal to noise ratio in dB
...
for r=1:length(SNR_dB)
    ...
    for reali=1:realization
        % generate channel
        H=sqrt(.5)*(randn(K,Nt)+1i*randn(K,Nt));
        ...
        Fzf=MRC_Down(H);
        Gzf= diag(ones(K,1))*sqrt(Snr);
        R_MRC(r,reali)=SumRate(H,Fzf,Gzf);
...
Legend3='Fully Digital MRC Precoder';
...
lgd=legend(Legend1,Legend2,Legend2);
% Axes Properties 
ax=gca;
