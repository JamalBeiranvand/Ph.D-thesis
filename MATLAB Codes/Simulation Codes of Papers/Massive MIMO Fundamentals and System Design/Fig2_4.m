...
SNR_dB = -20:4:20;             % Signal to noise ratio in dB
...
for r=1:length(SNR_dB)
    ...
    for reali=1:realization
        % generate channel
        H=sqrt(.5)*(randn(Nt,K)+1i*randn(Nt,K));
        
        % Full Digital MMSE beamforming
        Fmmse=MMSE_Up(H,Snr);
        Gmmse= diag(ones(K,1))*sqrt(Snr);
        R_MMSE(r,reali)=SumRate(H,Fmmse,Gmmse,'Link','Up');
           
        % Full Digital ZF beamforming
        Fzf=ZF_Up(H);
        Gzf= diag(ones(K,1))*sqrt(Snr);
        R_ZF(r,reali)=SumRate(H,Fzf,Gzf,'Link','Up');
        
        % Full Digital MRC beamforming
        Fzf=MRC_Up(H);
        Gzf= diag(ones(K,1))*sqrt(Snr);
        R_MRC(r,reali)=SumRate(H,Fzf,Gzf,'Link','Up');
...
Legend1='Fully Digital MMSE Receiver';
Legend2='Fully Digital ZF Receiver';
Legend3='Fully Digital MRC Receiver';
