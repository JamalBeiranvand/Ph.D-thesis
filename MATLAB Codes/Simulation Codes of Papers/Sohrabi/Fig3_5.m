...
Rxy = 32;                       % Number of the receiver antennas on y axis
...
SNR_dB = -15:5:10;             % Signal to noise ratio in dB
...
for r=1:length(SNR_dB)
    for reali=1:realization
        ...
        % Full Digital beamforming
         [WD,WRF,VRF,VD]=Sohrabi_Alg4_Full(H,NRF,Ns,SNR(r));
        ...
        for f=1:F
            ...
            Fopt = V(:,1:Ns);
            Wopt = U(:,1:Ns);
            R_Dig(r,reali) = R_Dig(r,reali) + log2(det(eye(Ns) + SNR(r)/Ns * pinv(Wopt) * H(:,:,f) * (Fopt) * Fopt' * H(:,:,f)' * Wopt))/F;
            ...
            Vt=VRF*VD(:,:,f);
            Wt=WRF*WD(:,:,f);
            R_Shr(r,reali) = R_Shr(r,reali)+log2(det(eye(Ns) +  pinv(Wt)  * H(:,:,f) * Vt  * (Vt)'  * H(:,:,f)' * Wt))/F;
            R_Shrp(r,reali) = R_Shrp(r,reali)+log2(det(eye(Ns) +  pinv(Wtp)  * H(:,:,f) * Vtp  * (Vtp)'  * H(:,:,f)' * Wtp))/F;
...
Data3= sum(R_Shrp,2)/realization;
%% plot 
Fig=figure;
hold on
P1=plot(Xaxis,Data1);
...
lgd=legend('Optimal Fully Digital BF','Proposed Hybrid BF Alg (fully-connected)','Proposed Hybrid BF Alg (partially-connected)');
