...
Ncl =20;                       % Number of Channel clusters(Scatters)
...
R_OMP3=zeros(length(SNR_dB),realization);
R_OMP4=zeros(length(SNR_dB),realization);
...
for r=1:length(SNR_dB)
    for reali=1:realization
        ...
        [WRF_OPM3,WBB_OPM3,FRF_OMP3, FBB_OMP3] = OMP_Alg(H,At,Ar,3,Ns,SNR(r));
        [WRF_OPM4,WBB_OPM4,FRF_OMP4, FBB_OMP4] = OMP_Alg(H,At,Ar,4,Ns,SNR(r));
        
        R_OMP(r,reali) = log2(det(eye(Ns) + SNR(r)/Ns * pinv(WRF_OPM * WBB_OPM) * H* FRF_OMP * (FBB_OMP) * FBB_OMP' * FRF_OMP' * H' * WRF_OPM * WBB_OPM));
        R_OMP3(r,reali) = log2(det(eye(Ns) + SNR(r)/Ns * pinv(WRF_OPM3 * WBB_OPM3) * H* FRF_OMP3 * (FBB_OMP3) * FBB_OMP3' * FRF_OMP3' * H' * WRF_OPM3 * WBB_OPM3));
        R_OMP4(r,reali) = log2(det(eye(Ns) + SNR(r)/Ns * pinv(WRF_OPM4 * WBB_OPM4) * H* FRF_OMP4 * (FBB_OMP4) * FBB_OMP4' * FRF_OMP4' * H' * WRF_OPM4 * WBB_OPM4));
...
Data4= sum(R_OMP3,2)/realization;
Data5= sum(R_OMP4,2)/realization;
%% plot 
Fig=figure;
hold on
P1=plot(Xaxis,Data1);
...
Legend2=['Proposed Alg.  $N^{RF}=$',num2str(NRF)];
Legend3=['Hybrid Beamforming in [10] (OMP).  $N^{RF}=$',num2str(NRF)];
Legend4='Hybrid Beamforming in [10] (OMP)  $N^{RF}=3$';
Legend5='Hybrid Beamforming in [10] (OMP)  $N^{RF}=4$';
...
% P3 properties
set(P5,'LineStyle','-.');           % '-' , '--' , ':' , '-.'
set(P5,'Marker','v');       % 'o' , '+'  , '*' , 'x' , 's' , '^' , 'v' , '>' , '<' , 'pentagram', 'hexagram'
set(P5,'LineWidth',2);
set(P5,'Color',[.6 .1 0]);
set(P5,'MarkerSize',7);
