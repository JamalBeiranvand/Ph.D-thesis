...
Txy = 128;                      % Number of the transmitter antennas on y axis
Rxz = 1;                       % Number of the receiver antennas on z axis
Rxy = 16;                       % Number of the receiver antennas on y axis
Ns  = 16;                       % Number of data streams
NRF = 16;                      % Number of RF chains
...
SNR_dB = 0:2:30;             % Signal to noise ratio in dB
...
R_Ana=zeros(length(SNR_dB),realization);
R_OMP=zeros(length(SNR_dB),realization);
R_SIC=zeros(length(SNR_dB),realization);
...
for reali=1:realization
   ...
   MM(reali)= mean(mean(abs(H).^2));

%     % Full Digital beamforming
%     [W_opt,F_opt]=DigitalBeamforming(H,Ns);

    %Analog Beamforming Algorithm
%     [WRF_Ana,FRF_Ana] = AnalogBeamforming(At,Ar,alpha,1);

    for r=1:length(SNR)
        
        % obtain FRF and WRF by function of SIC
        [FRF_SIC, FBB_SIC] = PartSIC(H, Ns, SNR(r));
%         FBB_SIC = sqrt(Ns)*FBB_SIC/norm(FRF_SIC*FBB_SIC,'fro');
        
        F2=propose_precoding(NRF,Nt,Nr,Nt/NRF,H,SNR(r));
        
%         sum(sum(abs(FRF_SIC*FBB_SIC)))
        R_SIC(r,reali) = log2(det(eye(Ns) + SNR(r)/Ns * H * FRF_SIC * FBB_SIC * (FBB_SIC)' * FRF_SIC' * H'));
        R_Dig(r,reali) = log2(det(eye(Ns) + SNR(r)/Ns * H * F2  * (F2)'  * H'));
    end
end
Data1= sum(R_Dig,2)/realization;
Data2= sum(R_SIC,2)/realization;
Data3= sum(R_Ana,2)/realization;
Xaxis=SNR_dB;
%% plot 
Fig=figure;
hold on
...
Legend2='SIC-Based Method';
% Legend3='Analog Beamforming';
