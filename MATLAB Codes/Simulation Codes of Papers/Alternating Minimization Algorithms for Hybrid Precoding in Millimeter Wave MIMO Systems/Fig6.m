...
NS  = [2 4 8];                       % Number of data streams                      % Number of RF chains
...
SNR_dB = -35:5:5;             % Signal to noise ratio in dB
...
R_PE=zeros(length(SNR_dB),realization);

Data1=zeros(length(SNR_dB),length(NS));
Data2=zeros(length(SNR_dB),length(NS));
Data3=zeros(length(SNR_dB),length(NS));
%% Main code
for n=1:length(NS)
    Ns=NS(n);
    NRF = Ns;
...
%         PE-AltMin algorithm
        [WRF_PE,WBB_PE,FRF_PE,FBB_PE] = PE_AltMin(H,NRF,Ns);


        for r=1:length(SNR)       
            R_PE(r,reali) = log2(det(eye(Ns) + SNR(r)/Ns * pinv(WRF_PE * WBB_PE)  * H * FRF_PE * FBB_PE * (FBB_PE)' * FRF_PE' * H' * WRF_PE * WBB_PE));
    ...
    Data2(:,n)= sum(R_MO,2)/realization;
    Data3(:,n)= sum(R_PE,2)/realization;
end
