...
SNR_dB = -30:5:5;             % Signal to noise ratio in dB
   ...
   [FRF_SDR,FBB_SDR] = PartSDR_AltMin(H,NRF,Ns);
    ...
    for r=1:length(SNR)
       ...
       [WRF_OPM,WBB_OPM,FRF_OMP,FBB_OMP] = OMP_Alg(H,At,Ar,NRF,Ns,SNR(r));
        ...
        R_Ana(r,reali) = log2(det(eye(Ns) + SNR(r)/Ns * pinv(WRF_Ana)  * H * FRF_Ana * (FRF_Ana)' * H' * WRF_Ana ));
        R_OMP(r,reali) = log2(det(eye(Ns) + SNR(r)/Ns * pinv(WRF_OPM * WBB_OPM)  * H * FRF_OMP * FBB_OMP * (FBB_OMP)' * FRF_OMP' * H' * WRF_OPM * WBB_OPM));
        R_SDR(r,reali) = log2(det(eye(Ns) + SNR(r)/Ns * pinv(W_opt)  * H * FRF_SDR * FBB_SDR * (FBB_SDR)' * FRF_SDR' * H' * W_opt));
        R_SIC(r,reali) = log2(det(eye(Ns) + SNR(r)/Ns * pinv(W_opt)  * H * FRF_SIC * FBB_SIC * (FBB_SIC)' * FRF_SIC' * H' * W_opt));
...
Data5= sum(R_SIC,2)/realization;
Data6= sum(R_Ana,2)/realization;
