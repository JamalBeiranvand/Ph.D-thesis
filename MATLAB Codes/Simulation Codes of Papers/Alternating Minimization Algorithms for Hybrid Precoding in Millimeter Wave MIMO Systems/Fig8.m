...
for r=1:length(SNR_dB)
    for reali=1:realization
        ...
        [WOPT,FOPT]=FD_OFDM_MIMO(H,Ns,SNR(r));
        [WDp,WRFp,VRFp,VDp]=OMP_OFDM_MIMO( H, NRF,Ns,SNR(r), At,Ar );
...
%             trace(Vt'*Vt)
            ...
            R_Dig(r,reali)  = R_Dig(r,reali) + log2(det(eye(Ns) +  pinv(Wopt) * H(:,:,f) * (Fopt) * Fopt'  * H(:,:,f)' * Wopt))/F;
            R_PE(r,reali)  = R_PE(r,reali) + log2(det(eye(Ns) +  pinv(Wt)   * H(:,:,f) *   Vt   * (Vt)'  * H(:,:,f)' * Wt))/F;
%             R_Shrp(r,reali) = R_Shrp(r,reali)+ log2(det(eye(Ns) +  pinv(Wts)  * H(:,:,f) *   Vts  * (Vts)' * H(:,:,f)' * Wts))/F;
            R_OMP(r,reali) = R_OMP(r,reali)+ log2(det(eye(Ns) +  pinv(Wtp)  * H(:,:,f) *   Vtp  * (Vtp)' * H(:,:,f)' * Wtp))/F;
        end             
     end
end
