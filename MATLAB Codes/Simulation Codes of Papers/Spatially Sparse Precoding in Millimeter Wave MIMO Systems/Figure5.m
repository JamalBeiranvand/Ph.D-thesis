...
for reali = 1:realization
    % generate channel
     [H,At,Ar,alpha]=Channel(Tx,Rx,Ncl,'Nray',Nray,'AngSpread',AngSpread,'TxSector',TxSector,'RxSector',RxSector);
     ...
     for r=1:length(SNR)
        ...
        % water filling
        lambda=diag( pinv(Wopt) * H * Fopt * (Fopt)' * H' * Wopt);
        g=WaterFilling1(lambda,SNR(r)); 
        G1=diag(g);
         LamOMP=diag(pinv(WRF_OPM * WBB_OPM)* H * FRF_OMP * (FBB_OMP) * FBB_OMP' * FRF_OMP' * H' * WRF_OPM * WBB_OPM);
         g2=WaterFilling(LamOMP,SNR(r)); 
         GOMP=diag(g2);
         R_opt(r,reali) = log2(det(eye(Ns) + G1 * pinv(Wopt) * H * Fopt * (Fopt)' * H' * Wopt));
         R_OMP(r,reali) = log2(det(eye(Ns) + GOMP * pinv(WRF_OPM * WBB_OPM)* H * FRF_OMP * (FBB_OMP) * FBB_OMP' * FRF_OMP' * H' * WRF_OPM * WBB_OPM));
%          R_OMP(r,reali) = log2(det(eye(Ns) + SNR(r)/Ns *  pinv(Wopt) * H * Fopt * (Fopt)' * H' * Wopt));
     end
end  
% prepare Data
Xaxis=SNR_dB;
Data1= sum(R_opt,2)/realization;
Data2= sum(R_OMP,2)/realization;
