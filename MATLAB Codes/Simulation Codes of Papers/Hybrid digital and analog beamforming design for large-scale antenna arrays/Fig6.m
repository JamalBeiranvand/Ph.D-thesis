...
for r=1:length(SNR_dB)
    ...
    for reali=1:realization
        ...
        % Sohrabi Algorithm b=1
        Nq=2^1;
        [VRF1,VD1]= Sohrabi_Alg3(H,NrfTx,Snr,Nq);
        R_Shr1(r,reali)=SumRate(H,VRF1*VD1,eye(K));
        
        % quantized-Proposed Algorithm 
        Nq=2^1;
        [VRF,VD]= Sohrabi_Alg3(H,NrfTx,Snr);
        VRFq=Qtz(VRF,Nq);
        VDhat=VRFq'*H'/(H*VRFq*(VRFq')*H');  %% from page 38
        qkk=diag((VDhat')*(VRFq)'*VRFq*VDhat);
        % waterfilling
        Pt=Snr;
        ...
        % end water filling
        P=diag(p./qkk);        
        VDq=VRFq'*H'*((H*VRFq*(VRFq')*H')\P.^(1/2));
        
        R_Shrq(r,reali)=SumRate(H,VRFq*VDq,eye(K));
    end
end
Data1= sum((R_ZF),2)/realization;
Data2= sum(R_Shr,2)/realization;
Data3= sum(R_Shr1,2)/realization;
Data4= sum(R_Shrq,2)/realization;
%% plot 
...
Legend2=['Proposed Alg.',' $b=\infty$, $N^{RF}=$',num2str(NrfTx)];
Legend3=['Proposed Alg.',' $b=1$, $N^{RF}=$',num2str(NrfTx)];
Legend4=['Quantized-Proposed Alg.',' $b=1$, $N^{RF}=$',num2str(NrfTx)];
